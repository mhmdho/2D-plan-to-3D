...
while True:
    ...
    if len(right_lines) == 0 or right_lines[-1][1][1] == max_p_y[1][1]:
        break
    min_p_y1 = min(right_lines, key=lambda a: a[0][0])
    min_p_y2 = min(right_lines, key=lambda a: a[1][0])
    min_p_y = min(min_p_y1, min_p_y2)
...
for entity in msp_e:
    if entity.dxftype() == 'LINE':
        entity.dxf.start = (entity.dxf.start.z, entity.dxf.start.y, -1 * entity.dxf.start.x - min_e_x - elev_plan_dif)
        entity.dxf.end = (entity.dxf.end.z, entity.dxf.end.y, -1 * entity.dxf.end.x - min_e_x - elev_plan_dif)
